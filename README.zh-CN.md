...
这是一个给 Krea2 / Krea2Edit 洗图工作流用的 ComfyUI 本地辅助插件。目标是把 Krea2 改造成更好用的“洗图流”：尽量保留原图比例、构图、人物姿势和身体轮廓，同时减少低 denoise 洗图时常见的皮肤黑点、斑点、色块、水印复制和脏纹理。

这个插件是独立本地插件，不修改 `ComfyUI-ConditioningKrea2Rebalance`。如果同时安装了上游 Krea2 Rebalance 插件，正常更新上游插件不会覆盖这里的节点。
...
检测参考图里的黑白或指定颜色高对比笔画，例如颜文字、贴纸、字幕、水印、彩色图标，并用局部颜色轻修。默认 `目标颜色模式=黑白`，也可以输入 `#RRGGBB` 或颜色名清理特定颜色。
...
- 这些清理节点是为了减少参考图进入 Krea2 前的脏点和符号继承，不是通用磨皮、人脸修复或完整 inpaint 工具。
