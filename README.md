...
- 参考图里有白色表情、贴纸、字幕、水印等高对比小结构，Krea2 容易把它们当成要保留的画面结构并扩散到新图里。
...
检测参考图里的亮白低饱和高对比笔画，例如白色颜文字、贴纸、字幕、水印，并用局部颜色轻修。建议放在 `Krea2洗图重平衡（肤质安全）` 和 `VAEEncode` 之前，避免 Krea2 视觉 Token 把这些小结构当成必须复制的内容。
...
- `白色阈值`：越低越容易识别浅色水印和白色符号，过低可能误伤高光。
...
```text
参考图 -> Krea2洗图参考尺寸 -> Krea2洗图白色符号水印清理 -> Krea2洗图Latent肤质清理 -> Krea2洗图皮肤暗点清理 -> VAEEncode -> KSampler
```

同时把 `Krea2洗图白色符号水印清理` 输出的清理图像接到 `Krea2洗图重平衡（肤质安全）`，用于帮助 Krea2 保持构图、姿势和人物比例。不要把带水印、贴纸或明显符号的原图直接接进 Krea2 视觉参考，否则模型容易复制甚至扩散这些小结构。
